...
(run-with-idle-timer (* 5 60) 1 #'save-all)
;; 5 分钟保存一次也比 focus-out-hook 保存要好得多。因为 focus-out-hook 保存会自动修改自己刚刚的格式，而我只需要一个不长的时间保留那个格式即可
...
(use-package! org
  ...
  (progn
    ...
    ;;                (file+headline "~/Documents/org/notes.org" "Todos")
    ;;                "* TODO %?\n%i\n%u\n%a" :prepend t))
    ...
    (add-to-list 'org-capture-templates
                 '("ws" "Code Snippet" entry (file "~/Documents/org/notes.org")
...
;; org archive file
(setq org-archive-location "~/Documents/org/notes.org::* Archived")
